	...
	public class Client : IDisposable, SocketIOClient.IClient
	{
				...
				if (!(this.ReadyState == WebSocketState.Connecting || this.ReadyState == WebSocketState.Open))
				{
					try
					{
						...
						if (this.HandShake == null || string.IsNullOrEmpty(this.HandShake.SID) || this.HandShake.HadError)
						{
							...
							{
								r = " this.HandShake == null";
							}
							else
							{
								r = " " + this.HandShake.ErrorMessage;

								if(string.IsNullOrEmpty(this.HandShake.SID))
								{
									r += " string.IsNullOrEmpty(this.HandShake.SID)";
								}

								if(this.HandShake.HadError)
								{
									r += " this.HandShake.HadError";
								}
							}



							this.LastErrorMessage = string.Format("Error initializing handshake with {0}" + r, uri.ToString());
						...
						else
						{
							...
							throw new Exception("Manual Error 1");
							...
							throw new Exception("Manual Error 2");
							this.wsClient.Opened += this.wsClient_OpenEvent;
							throw new Exception("Manual Error 3");
							this.wsClient.MessageReceived += this.wsClient_MessageReceived;
							throw new Exception("Manual Error 4");
							this.wsClient.Error += this.wsClient_Error;
							throw new Exception("Manual Error 5");
							this.wsClient.Closed += wsClient_Closed;
							throw new Exception("Manual Error 6");
							this.wsClient.Open();
							throw new Exception("Manual Error 7");
						}
					}
					catch (Exception ex)
					{
		...
		protected SocketIOHandshake requestHandshake(Uri uri)
		{
			...
			{ 
				try
				{
					...
					if (string.IsNullOrEmpty(value))
						...
						UnityEngine.Debug.Log("DownloadString --> " + value.Remove(0,5));
				}
				catch (Exception ex)
				{
			...
			if (string.IsNullOrEmpty(errorText))
				handshake = SocketIOHandshake.LoadFromJsonString(value);
			else
			{
